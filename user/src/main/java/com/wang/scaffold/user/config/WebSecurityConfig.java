...
// 作用：使得使用 @ConfigurationProperties 注解的类生效，即将 JwtProperties类注入到Spring 容器中。
...
public class WebSecurityConfig {
    ...
    public AuthenticationManager authenticationManager() {
        ...
        try {
            ...
            // 添加手机号码登录的验证类
            managerBuilder.authenticationProvider(phoneCodeAuthenticationProvider);
...
//	}
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.cors().and() // 开启跨域资源共享
                .csrf().disable() // 关闭 csrf
                .authorizeRequests() // 开启 对url进行访问权限控制
                .antMatchers(BYPASS_PATTERNS).permitAll() // 配置过的路由，完全绕过了spring security的所有filter，相当于不走spring security
                .and()
                .authorizeRequests()
                ...
                .sessionManagement()
                .sessionCreationPolicy(SessionCreationPolicy.STATELESS); // 因为使用了JWT 所以关闭Session
        return http.build();
    }
